...
//
// FIXME: Because TestingSetup is initialized only once at the beginning of the fuzzing setup, this test has
// non-determinism. Depending on the ordering of the corpus, an input may create a certain block and
// modify the underlying chainstate and a following input may reference this block as hashPrevBlock.
// If we were to swap the ordering, the coverage signal would be wildly different and therefore useless.
// However, if we initiate TestingSetup each iteration, the number of executions (though more deterministic),
// slows to a crawl.
